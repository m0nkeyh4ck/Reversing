...
int main() {
    ...
    const std::string proceso = "reto2.exe"; // Nombre del proceso que estamos buscando
    const DWORD direccion_virtual = 0x003f9487; // Dirección en memoria donde está la instrucción JZ
    const BYTE nuevo_bytes[2] = { 0x0F, 0x85 };
    ...
    if (!WriteProcessMemory(hProceso, (LPVOID)direccion_virtual, nuevo_bytes, sizeof(nuevo_bytes), nullptr)) {
        CloseHandle(hProceso);
